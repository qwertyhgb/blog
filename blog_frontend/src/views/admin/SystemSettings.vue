...
const siteRules = {
  ...
  siteDescription: [{ max: 200, message: "描述最多200个字符", trigger: "blur" }],
  siteKeywords: [{ max: 100, message: "关键词最多100个字符", trigger: "blur" }],
  siteAuthor: [
    { required: true, message: "请输入作者名称", trigger: "blur" },
    { min: 2, max: 20, message: "长度在 2 到 20 个字符", trigger: "blur" },
  ],
  siteUrl: [{ required: true, message: "请输入网站URL", trigger: "blur" }],
  pageSize: [{ required: true, message: "请输入每页显示数量", trigger: "blur" }],
};

const systemRules = {
  uploadMaxSize: [{ required: true, message: "请输入上传文件大小限制", trigger: "blur" }],
  uploadAllowedTypes: [{ required: true, message: "请输入允许上传的文件类型", trigger: "blur" }],
  emailPort: [
    {
      type: "number" as const,
      min: 1,
      max: 65535,
      message: "端口在 1 到 65535 之间",
      trigger: "blur",
    },
  ],
};
        ...
        <n-tab-pane name="system" tab="系统设置">
          ...
          >
            ...
            <n-form-item label="允许上传类型" path="uploadAllowedTypes">
              <n-input
                v-model:value="systemSettings.uploadAllowedTypes"
                placeholder="请输入允许上传的文件类型，多个类型用逗号分隔"
              />
            </n-form-item>
            ...
            <n-form-item label="邮件服务器">
              <n-input
                v-model:value="systemSettings.emailHost"
            ...
            <n-form-item label="邮件用户名">
              <n-input
                v-model:value="systemSettings.emailUsername"
            ...
            <n-form-item label="邮件密码">
              <n-input
                v-model:value="systemSettings.emailPassword"
                type="password"
                placeholder="请输入邮件密码"
              />
            </n-form-item>
